...
{
	//checking what we draw
	switch (statePen)
	{
	case 1://draw line
		if (points->Count > 1) {
			line obj(color, int(mousePosition.X), int(mousePosition.Y), size, points);
			obj.drawLines(this->canvas);
		}
		break;
	...
	case 2://draw square
		...
		{
			square obj(color, int(mousePosition.X), int(mousePosition.Y), int(numericUpDownWidthSquare->Value), int(numericUpDownHeightSquare->Value), size);
			obj.drawRectangle(this->canvas);
		}
		break;
	...
	case 3://draw circle
		if (mousePosition.X != -1 && mousePosition.Y != -1)
		{
			circle obj(color, int(mousePosition.X), int(mousePosition.Y), int(numericUpDownWidthCircle->Value), int(numericUpDownHeightCircle->Value), size);
			obj.drawCircle(this->canvas);
		}			
		break;
	...
	case 4://draw texture
		if (mousePosition.X != -1 && mousePosition.Y != -1)
		{
			Texture obj(color, mousePosition.X, mousePosition.Y, texture->Width, texture->Height,size, texture);
			obj.drawTexture(this->canvas);
		}
		break;
